...
export const WidgetContent: React.FC = () => {
  ...
  return (
    <div className="flex-1 p-2 flex flex-col justify-between text-xs bg-slate-900/10 min-h-0">
      ...
      {/* Center content: Settings panel OR Searchable AI output OR simulated CPU telemetry */}
      <div className="flex-1 min-h-0 flex flex-col justify-center my-0.5">
        <div className="flex-1 flex flex-col min-h-0">
          ...
          {conversationHistory.length > 0 && (
            <div className="flex gap-1 mb-1.5 items-center select-none">
              ...
              <button
                ...
                className="h-5 px-1 bg-slate-950/60 border border-slate-800/40 hover:bg-cyan-950/20 text-slate-400 hover:text-cyan-400 text-[8px] font-bold rounded cursor-pointer"
          ...
          <div 
            ref={scrollRef}
            className="flex-1 overflow-y-auto px-1.5 py-1 bg-slate-950/40 border border-slate-800/40 rounded-lg text-[10px] leading-relaxed text-slate-300 font-medium select-text"
          >
            <CapturePreview />

            {/* If history is empty and no active response, render the CPU bar and the Quick Guide */}
            {conversationHistory.length === 0 && !response && !loading && !error && !screenshotError && (
            ...
            {/* Render dynamic conversation history logs */}
            {filteredHistory.map((msg: any) => (
              <div key={msg.id} className="mb-2 p-1.5 bg-slate-900/30 border border-slate-850/40 rounded-md relative group select-text">
                ...
                <div className="whitespace-pre-wrap font-sans text-slate-300 text-[9px]">
                  {msg.content}
                </div>
              </div>
            ))}
            ...
            {streaming && response && (
              <div className="mb-2 p-1.5 bg-cyan-950/10 border border-cyan-900/20 rounded-md select-text">
                ...
                <div className="whitespace-pre-wrap font-sans text-cyan-200 text-[9px]">
                  {response}
                </div>
              </div>
            )}
      ...
      <form onSubmit={handleSubmit} className="flex gap-1 items-center mt-1 select-none">
        ...
        <button
          ...
          className="h-6 w-6 flex items-center justify-center bg-slate-950/40 border border-slate-800/40 hover:bg-rose-950/20 hover:border-rose-900/40 text-slate-400 hover:text-rose-400 text-[9px] font-bold rounded-md cursor-pointer transition-all active:scale-95"
